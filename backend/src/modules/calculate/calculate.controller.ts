...
const UV_INDEX: Record<TimeSlot, number> = {
...
function calcPF(components: ProtectionComponent[]): number {
  ...
  const pf = (spf             > 0 ? ((1 - 1/spf) * 100) * 0.25         : 0)
...
function calcSafeTime(skinType: SkinType, outdoorTime: TimeSlot, components: ProtectionComponent[]) {
  const uv = UV_INDEX[outdoorTime]
  const medBase = (SKIN_FACTOR[skinType] * 133.33) / uv
  const pf = calcPF(components)
  const multiplier = 1 + pf / 100
  return {
    safeOutdoorMinutes: Math.round(medBase * multiplier),
...
export async function createCalculation(req: Request, res: Response): Promise<void> {
  ...
  const { safeOutdoorMinutes, protectionScore } = calcSafeTime(skinType, outdoorTime, protectionComponents)
  const row = await prisma.calculation.create({
    data: {
      skinType, outdoorTime,
      protectionItems: JSON.stringify(protectionComponents),
      protectionScore, safeOutdoorMinutes,
    },
  })
...
export async function updateCalculation(req: Request, res: Response): Promise<void> {
  ...
  const remainingSeconds = body.remainingSeconds !== undefined
    ? body.remainingSeconds : existing.remainingSeconds
  ...
  const row = await prisma.calculation.update({
    ...
    data: {
      skinType, outdoorTime,
      protectionItems: JSON.stringify(protectionComponents),
      protectionScore, safeOutdoorMinutes, remainingSeconds,
    },
  })
  res.json(formatRow(row))
}
