  ...
  if (!hasActivities || !equipment) {
    return (
      <div className="flex flex-col items-center justify-center py-10 text-orange-300">
        ...
        <p className="text-sm font-semibold text-orange-300">Recommendations will appear here</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        ...
        <span className="bg-amber-400 text-white text-xs font-bold rounded-full px-3 py-0.5">
          {equipment.level}
        </span>
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        {equipment.items.map((item) => (
          <div
            key={item}
            className="flex items-center gap-1.5 bg-white border border-amber-200 rounded-xl px-3 py-2 shadow-sm"
          >
